...
UART_RX_EULER_ACCx_FLAG =    np.uint64(1 << 1)
UART_RX_EULER_RAWx_FLAG =    np.uint64(1 << 2)
UART_RX_EULER_MAGx_FLAG =    np.uint64(1 << 3)
UART_RX_CUR_ALLx_FLAG =      np.uint64(1 << 4)
UART_RX_CUR_LIGHT1x_FLAG =   np.uint64(1 << 5)
UART_RX_CUR_LIGHT2x_FLAG =   np.uint64(1 << 6)
UART_RX_VOLTS24x_FLAG =      np.uint64(1 << 7)

UART_TX_DES_MOTORSx_FLAG =       np.uint64(1 << 0)
UART_TX_DES_LIGHTx_FLAG =        np.uint64(1 << 1)
UART_TX_DES_CAM_ANGLEx_FLAG =    np.uint64(1 << 2)
...
class UART_Xfer_Container:
    ...
    def _parse_rx_buffer(self):
        ...
        try:
            ...
            print("UART RX WRONG PACKET")
            return False
        self.rx_refresh_flags = np.uint64(rxPacket[1])
        self.euler = [rxPacket[2], rxPacket[3], rxPacket[4]] if self.rx_refresh_flags & UART_RX_EULERx_FLAG else None
        self.eulerAcc = [rxPacket[5], rxPacket[6], rxPacket[7]] if self.rx_refresh_flags & UART_RX_EULER_ACCx_FLAG else None
        self.eulerRaw = [rxPacket[8], rxPacket[9], rxPacket[10]] if self.rx_refresh_flags & UART_RX_EULER_RAWx_FLAG else None
        self.eulerMag = [rxPacket[11], rxPacket[12], rxPacket[13]] if self.rx_refresh_flags & UART_RX_EULER_MAGx_FLAG else None
        self.currentAll = rxPacket[14] if self.rx_refresh_flags & UART_RX_CUR_ALLx_FLAG else None
        self.cirrentLight1 = rxPacket[15] if self.rx_refresh_flags & UART_RX_CUR_LIGHT1x_FLAG else None
        self.cirrentLight2 = rxPacket[16] if self.rx_refresh_flags & UART_RX_CUR_LIGHT2x_FLAG else None
        self.voltage24 = rxPacket[17] if self.rx_refresh_flags & UART_RX_VOLTS24x_FLAG else None
        return True
    ...
    def set_mots_values(self, values):
        self.des_mot = values
        self.tx_refresh_flags |= UART_TX_DES_MOTORSx_FLAG
    
    def set_lights_values(self, values):
        self.des_lights = values
        self.tx_refresh_flags |= UART_TX_DES_LIGHTx_FLAG
        
    def set_cam_angle_value(self, value):
        self.des_cam_angle = value
        self.tx_refresh_flags |= UART_TX_DES_CAM_ANGLEx_FLAG

    # Getters
    def get_IMU_angles(self):
        if self.rx_refresh_flags & UART_RX_EULERx_FLAG:
            self.rx_refresh_flags &= ~(UART_RX_EULERx_FLAG)
            return self.euler
        return None
    
    def get_IMU_raw(self):
        if self.rx_refresh_flags & UART_RX_EULER_RAWx_FLAG:
            self.rx_refresh_flags &= ~(UART_RX_EULER_RAWx_FLAG)
            return self.eulerRaw
        return None
    
    def get_IMU_accelerometer(self):
        if self.rx_refresh_flags & UART_RX_EULER_ACCx_FLAG:
            self.rx_refresh_flags &= ~(UART_RX_EULER_ACCx_FLAG)
            return self.eulerAcc
        return None
    
    def get_IMU_magnetometer(self):
        if self.rx_refresh_flags & UART_RX_EULER_MAGx_FLAG:
            self.rx_refresh_flags &= ~(UART_RX_EULER_MAGx_FLAG)
            return self.eulerMag
        return None
    
    def get_current_all(self):
        if self.rx_refresh_flags & UART_RX_CUR_ALLx_FLAG:
            self.rx_refresh_flags &= ~(UART_RX_CUR_ALLx_FLAG)
    ...
    def get_current_lights(self):
        light1 = None
        light2 = None
        if self.rx_refresh_flags & UART_RX_CUR_LIGHT1x_FLAG:
            self.rx_refresh_flags &= ~(UART_RX_CUR_LIGHT1x_FLAG)
            light1 =  self.cirrentLight1
            
        if self.rx_refresh_flags & UART_RX_CUR_LIGHT2x_FLAG:
            self.rx_refresh_flags &= ~(UART_RX_CUR_LIGHT2x_FLAG)
    ...
    def get_voltage(self):
        if self.rx_refresh_flags & UART_RX_VOLTS24x_FLAG:
            self.rx_refresh_flags &= ~(UART_RX_VOLTS24x_FLAG)
